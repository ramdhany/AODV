...
public abstract class CFMetaInterface implements ICFMetaInterface, ILifeCycle, IUnknown, IMetaInterface{
    ...
    //optional reference to a remote visualisation client
    // - used from the methods create_component, delete_component, local_bind, expose_interface and expose_receptacle
    ...
    public CFMetaInterface(IUnknown pRuntime) {
        ...
        //check if OCJ is running a remote visualisation client
        IDebug debugIntf = (IDebug) m_PSR_IOpenCOM.m_pIntf.QueryInterface("OpenCOM.IDebug");
        if (debugIntf != null)
           rvClient = debugIntf.getRVClient();
    ...
    */
    ...
    public IUnknown create_component(String componentType, String componentName) {
        ...
        if (rvClient != null)
           rvClient.creatingComponentInFramework(m_PSR_IOpenCOM.m_pIntf.getComponentName((IUnknown) this), componentName);
   ...
   if (rvClient != null)
      rvClient.interfaceExposed(m_PSR_IOpenCOM.m_pIntf.getComponentName((IUnknown) this), rintf);
	...
	for(int index=0; index<m_prElems; index++){
		if (((ExposedReceptacle) m_recpseq.get(index)).Intf==rintf){
			inCFInts=true;
			break;
		}
	}
	...
	m_prElems++;
        ...
        if (rvClient != null)
            rvClient.receptacleExposed(m_PSR_IOpenCOM.m_pIntf.getComponentName((IUnknown) this), rintf);
	...
	m_piElems--;
        ...
        if (rvClient != null)
            rvClient.interfaceRemove(m_PSR_IOpenCOM.m_pIntf.getComponentName((IUnknown) this), rintf);
	...
	m_prElems--;
        ...
        if (rvClient != null)
            rvClient.receptacleRemoved(m_PSR_IOpenCOM.m_pIntf.getComponentName((IUnknown) this), rintf);
    ...
    */
    ...
    public boolean commit_arch_transaction() {
        ...
        if(m_PSR_IAccept.m_pIntf==null){
            CFlock.release();
            return true;
        }
            
        // Call the operation to check the new configuration.
        if(((IAccept) m_PSR_IAccept.m_pIntf).isValid(m_ppComps,m_intseq, m_pcElems , m_piElems)){
            CFlock.release();
            return true; 
        }
        else{
