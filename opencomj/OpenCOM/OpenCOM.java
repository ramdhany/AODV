...
import OpenCOM.Visualisation.VisualGraph;
...
public class OpenCOM implements IOpenCOM, IDebug, IMetaArchitecture, IUnknown, IMetaInterception{
    ...
    // Runtime state
    Vector<OCM_GraphNode_t> mGraph;                 //!< System Graph
    long mcGraphNodes;                              //!< Number of components in system
    long mcConnID;                                  //!< Unique connection Identifer counter
    MetaInterface Meta;
    
    /**
     * Remote reference to the visualisation server for this runtime
     */
    RemoteVisClient rvClient;
    ...
    */
    public synchronized long connect(IUnknown pIUnkSource, IUnknown pIUnkSink, String iid) {
        ...
        //if vis. client running, send this info to it to be forwarded to a remote visualization sever
        if (rvClient != null)
           rvClient.connectionMade(getComponentName(pIUnkSource), getComponentName(pIUnkSink), iid);
    ...
    */
    boolean registerConnection(IUnknown pIUnkSource, IUnknown pIUnkSink, String riid, long connID){
    ...
    */
    public synchronized Object createInstance(String componentType, String componentName) {
        ...
        try{
            ...
            System.err.println("Error Introspecting component interfaces "+e);
        }
        ...
        if (rvClient != null)
           rvClient.componentCreated((IUnknown) pIUnknown, componentName);
        } 
        else
            return null;
        ...
        for (int i = 0; i < mGraph.size(); i++){
            ...
            if (((OCM_GraphNode_t)mGraph.elementAt(i)).pIUnknown.hashCode() == pCompToDelete.hashCode()){
                ...
                if (pIConnections == null){
                       ...
                       if (rvClient != null)
                          rvClient.componentDeleted((IUnknown) pCompToDelete, getComponentName(pCompToDelete));
                ...
                for (int index = 0; index < recpListVector.size(); index++){
                        pIConnections.disconnect(recpListVector.elementAt(index).iid, recpListVector.elementAt(index).connID);
                        deRegisterConnection(recpListVector.elementAt(index).connID);
                }
                ...
                for (int index = 0; index < IntfListVector.size(); index++){
                        pParent = IntfListVector.elementAt(index).sourceIndex;
                        pIntfConnections = (IConnections)pParent.QueryInterface("OpenCOM.IConnections");
                        if (pIntfConnections.disconnect(IntfListVector.elementAt(index).iidName, IntfListVector.elementAt(index).connID))
                                deRegisterConnection(IntfListVector.elementAt(index).connID);
                }
    ...
    */ 
    public synchronized boolean disconnect(long connID) {
           ...
           if (rvClient != null)
              rvClient.connectionBroken(pConnInfo.sourceComponentName, pConnInfo.sinkComponentName, pConnInfo.interfaceType);
