...
public class MetaInterface{
    ...
    public int enumRecps(IUnknown compRef, java.util.Vector<OCM_RecpMetaInfo_t> recpList) {
        ...
        for (int i = 0; i < publicFields.length; i++) {
            ...
            if(fieldType.toString().equalsIgnoreCase("OpenCOM.OCM_MultiReceptacleDynamicContext")){
                // add to vector
                 type= "OCM_MULTI_RECEPTACLE";
            }
            if(type!=null){
                try{
                    // Extract the 
                    Object Value=publicFields[i].get(compRef);
    ...
    public boolean SetAttributeValue(String iid, String Kind, String Name, String Type, Object Value){
        if(Kind.equalsIgnoreCase("Interface")){
            ...
            for (int i = 0; i < publicFields.length; i++) {
                ...
                if((type.equalsIgnoreCase("OpenCOM.OCM_SingleReceptacle"))||(type.equalsIgnoreCase("OpenCOM.OCM_MultiReceptacle"))
                ||(type.equalsIgnoreCase("OpenCOM.OCM_MultiReceptacleContext"))||(type.equalsIgnoreCase("OpenCOM.OCM_MultiReceptacleParallel"))){
                    try{
                        IReceptacle recp = (IReceptacle) publicFields[i].get(m_Comp);
                        OCM_SingleReceptacle rt = (OCM_SingleReceptacle) publicFields[i].get(m_Comp);
                        Class SR = rt.getClass();
                        Field[] publicFields2 = SR.getFields();
                        String type2 = (String) publicFields2[0].get(rt);
                        if(type2.equalsIgnoreCase("interface "+iid)){
