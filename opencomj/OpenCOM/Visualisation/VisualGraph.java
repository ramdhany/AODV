...
public class VisualGraph extends Frame{
    ...
    static final long serialVersionUID = -2506253878204746386L;
    ...
    public Vector<VisualGraph> subWindowList;
     ...
     */
    public VisualGraph(IOpenCOM pIOpenCOM, String FrameworkTitle){
        try{
    ...
    public VisualGraph(String FrameworkTitle)
       {
        try{
            ...
            subWindowList = new Vector<VisualGraph>();
            jbInit(FrameworkTitle);
        }catch(Exception e){
            e.printStackTrace();
        }
   ...
   public void componentCreated(String componentName, String[] interfaces, String[] receptacles, String inFramework)
      ...
      if (componentName != null)
         ...
         if (inFramework != null)
            {
            VisualGraph frameworkGraph = findFrameworkGraph(inFramework);
   ...
   public VisualGraph findFrameworkGraph(String name)
      {
   ...
   public void frameworkCreated(String componentName, String[] interfaces, String[] receptacles, String inFramework)
      ...
      if (componentName != null)
         ...
         VisualGraph ng = new VisualGraph(componentName + sourceAddress);
   ...
   public void interfaceRemoved(String componentName, String interfaceName, String inFramework)
      {
          System.out.println("Removing  "+interfaceName+" ... from "+componentName+" in Framework "+inFramework);
      //find the framework, then the component, and add the interface to it
      if (inFramework != null)
         {
         VisualGraph frameworkGraph = findFrameworkGraph(inFramework);
         ...
         Component c = frameworkGraph.panel.findComponent(componentName);
         c.removeInterface(interfaceName);
         }
         else
         {
         Component c = panel.findComponent(componentName);
   ...
   public void receptacleExposed(String componentName, String receptacleName, String inFramework)
      ...
      if (inFramework != null)
         ...
         Component c = frameworkGraph.panel.findComponent(componentName);
         c.addReceptacle(receptacleName);
         }
         else
         {
         Component c = panel.findComponent(componentName);
   ...
   public void receptacleRemoved(String componentName, String receptacleName, String inFramework)
      {
          System.out.println("Removing  "+receptacleName+" ... from "+componentName+" in Framework "+inFramework);
      //find the framework, then the component, and add the interface to it
      if (inFramework != null)
         {
         VisualGraph frameworkGraph = findFrameworkGraph(inFramework);
         ...
         Component c = frameworkGraph.panel.findComponent(componentName);
         c.removeReceptacle(receptacleName);
         }
         else
         {
         Component c = panel.findComponent(componentName);
...
public class GraphPanel extends Panel implements Runnable, MouseListener, MouseMotionListener {
    ...
    static final long serialVersionUID = -3391467219589501222L;
     ...
     */
   public Component addComponent(String lbl, IUnknown pComp) {
        Dimension d = getSize();
        double x= (d.width/2);   
        double y= (d.height/2);  
   ...
   public Component addFramework(String lbl, IUnknown pMeta) {
     Dimension d = getSize();
     double x= (d.width/2);   
	   double y= (d.height/2);  
...
public synchronized void removeInterface(String interfaceName)
   {
       for(int i=0; i<Interfaces.size(); i++){
           if(interfaceName.equalsIgnoreCase(Interfaces.get(i))){
               Interfaces.remove(i);
               break;
           }
       }
       ...
       for(int i=0; i<IntfPoints.size(); i++){
           point a = IntfPoints.get(i);
           if(result.equalsIgnoreCase(a.name)){
               IntfPoints.remove(i);
               break;
           }
       }
...
public synchronized void removeReceptacle(String receptacleName)
   {
       for(int i=0; i<Receptacles.size(); i++){
           if(receptacleName.equalsIgnoreCase(Receptacles.get(i))){
               Receptacles.remove(i);
               break;
           }
       }
       StringTokenizer st = new StringTokenizer(receptacleName,".");
